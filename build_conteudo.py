...
for i,par in enumerate(rx_sets):
    ...
    for nome,img,vert in par:
        # cima = bicho e baixo = CHAPA DE RAIO-X ALINHADOS na mesma tela-padrao
        # (mv_<bicho>_rx e mv_<bicho>_xray, 800x592, sujeito na MESMA caixa) para
        # o esqueleto CASAR de tamanho e posicao com o bicho quando a janelinha
        # revela. Vertebrado mostra esqueleto; invertebrado, corpo mole sem ossos.
        itens.append({"k":img,"cima":img+"_rx","baixo":img+"_xray","borrado":False,
            ...
            "d":["Leve a janelinha bem devagar pelo meio do bicho.",
                 "Procure uma linha de ossos no meio das costas (a coluna).",
